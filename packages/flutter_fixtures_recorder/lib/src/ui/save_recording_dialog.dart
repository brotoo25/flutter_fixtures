...
/// running. Saves are confirmed with a snackbar naming the session; a save
/// the store rejects is reported in a snackbar too, and the recording stays
/// in place so the user can retry or discard it.
///
...
Future<RecordingSession?> stopRecordingWithPrompt(
  ...
  try {
    session = await recorder.stopRecording(
      name: choice.name,
      discard: choice.discard,
    );
  } catch (e) {
    // The recorder kept the recording; tell the user and let them retry.
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
