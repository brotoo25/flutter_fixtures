...
import 'openapi_schema_sampler.dart';
...
class OpenApiFixtureSource implements HttpFixtureSource {
  ...
  List<_DocumentDraft> _documentsFor(
    ...
    final sample = _samplerFor(spec).sample(mediaType['schema']);
  ...
  Map<String, dynamic> _deref(
    ...
    while (true) {
      ...
      final resolved = _samplerFor(spec).resolvePointer(ref);
      if (resolved is! Map) {
        return current;
      }
